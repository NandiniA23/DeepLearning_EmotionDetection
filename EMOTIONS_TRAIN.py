...
import cv2 												#
##############################################TRAINING DATA##############################################
dataset = np.load("Datasets/EMOTIONS.npy",allow_pickle = True)								#Cat Dog dataset.
X = [] 													#Training inputs.
Y = [] 													#Training Targets.
...
X = np.array(X).reshape(-1,48,48,3) 											#Converting the input list to a numpy array.
...
model.add(Dropout(0.2)) 											#Neuron dropout probability is 30%.
...
model.add(Dropout(0.2))											#Neuron dropout probability is 30%.
model.add(MaxPool2D(pool_size=(2,2)))#Maxpooling layer reduces the size of the feature maps by half.
model.add(Conv2D(512,(3,3),padding='same'))									#Convolutional layer with 32 feature maps and 3x3 kernels.
model.add(Activation('relu')) 											#ReLU activation function.
model.add(Dropout(0.2))											#Neuron dropout probability is 30%.
model.add(MaxPool2D(pool_size=(2,2)))
model.add(Conv2D(1024,(3,3),padding='same'))									#Convolutional layer with 32 feature maps and 3x3 kernels.
model.add(Activation('relu')) 											#ReLU activation function.
model.add(Dropout(0.2))											#Neuron dropout probability is 30%.
...
filepath="Models/EMOTIONS/weights-improvement-{epoch:02d}-{val_accuracy:.2f}.hdf5" 					#Path to save the checkpoints.
...
model.fit(X, Y, validation_split=0.30, epochs=25, batch_size=64, callbacks=callbacks_list, verbose=1)				#Train the model for 30 epochs using 30% of the data as validation data.############################################SAVING THE MODEL############################################
#########################################################################################################
